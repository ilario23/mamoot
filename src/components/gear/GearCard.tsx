  ...
  return (
    <div className='relative group'>
      ...
      >
        ...
        <div
          className={`p-3 md:p-4 space-y-2 md:space-y-3 ${retired ? 'opacity-50' : ''}`}
        >
          {/* Name + primary badge + distance — inline on mobile */}
          <div className='flex items-start justify-between gap-2'>
            <div className='min-w-0 flex-1'>
              <h3
                className='font-black text-xs md:text-sm leading-tight truncate'
                title={gear.name}
              >
                {gear.name}
              </h3>
          ...
          <button
            ...
            aria-label={
              retired ? `Restore ${gear.name}` : `Retire ${gear.name}`
            }
            tabIndex={0}
          >
            {retired ? (
