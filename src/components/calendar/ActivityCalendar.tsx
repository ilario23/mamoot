...
const ActivityCalendar = ({ activities, year }: ActivityCalendarProps) => {
  ...
  const { weeks, monthLabels } = useMemo(() => {
    ...
    const activityMap: Record<string, Array<ActivitySummary & { level: number }>> = {};
    activities.forEach((activity) => {
      ...
      activityMap[dateKey].push({ ...activity, level });
    });
  ...
  }, [activities, activitiesByType, year]);
  ...
  return (
    <div className="border-3 border-foreground bg-background shadow-neo">
      ...
      <div className="p-5 overflow-x-auto">
          ...
          <div className="flex" style={{ gap: CELL_GAP }}>
            {weeks.map((week, weekIdx) => (
              <div
                ...
                style={{ gap: CELL_GAP }}
              >
                {/* Pad incomplete first/last weeks */}
                {week.map((dayData) => {
                  // Map day to row: Mon=0, Tue=1, ... Sun=6
                  const jsDay = getDay(dayData.date); // 0=Sun, 1=Mon, ...
        ...
        {/* Legend */}
        <div className="flex flex-wrap gap-4 mt-5 pt-4 border-t border-muted">
