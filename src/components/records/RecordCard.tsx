...
const RecordCard = ({ record, bucket, activityType }: RecordCardProps) => {
  if (!record) {
    return (
      <div className="border-3 border-border bg-background p-3 md:p-5 shadow-neo opacity-50">
        <p className="text-[10px] md:text-xs font-black uppercase tracking-wider text-muted-foreground mb-1.5 md:mb-2">
          {bucket.label}
        </p>
        <p className="text-xl md:text-2xl font-black text-muted-foreground">--:--</p>
        <p className="text-xs md:text-sm font-bold text-muted-foreground mt-1">
          No record yet
        </p>
        <div className="h-1.5 md:h-2 w-12 md:w-16 mt-2 md:mt-3 bg-muted" />
      </div>
    );
  }
    ...
    <div className="border-3 border-border bg-background p-3 md:p-5 shadow-neo hover:shadow-neo-lg transition-shadow">
      ...
      <div className="flex items-center justify-between mb-2 md:mb-3">
        ...
        <div className="flex items-center gap-1">
          {isPR && (
            <span className="text-[8px] md:text-[10px] font-black uppercase bg-accent text-foreground px-1 md:px-1.5 py-0.5 border-2 border-border">
              PR
            </span>
          )}
          <Trophy className="h-3.5 w-3.5 md:h-4 md:w-4 text-accent" aria-hidden="true" />
        </div>
      </div>
      ...
      {/* Stats grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-1 md:gap-y-2 mt-2 md:mt-4">
        <div className="flex items-center gap-1.5">
          <Gauge className="h-3 w-3 md:h-3.5 md:w-3.5 text-muted-foreground shrink-0" aria-hidden="true" />
          <span className="text-xs md:text-sm font-bold">
            {formatPace(pace)}/km
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <Calendar className="h-3 w-3 md:h-3.5 md:w-3.5 text-muted-foreground shrink-0" aria-hidden="true" />
          <span className="text-xs md:text-sm font-bold">{recordDate}</span>
        </div>
      ...
      {/* Activity name — hidden on mobile for compactness */}
      <p className="hidden md:block text-xs font-bold text-muted-foreground mt-3 truncate">
        {effort.activityName}
      </p>

      {/* Accent bar */}
      <div className={`h-1.5 md:h-2 w-12 md:w-16 mt-2 md:mt-3 ${ACCENT_CLASS[activityType]}`} />
    </div>
  );
};
